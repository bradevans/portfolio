var result, color, circle, clip, theG, circleY, drawCount = 0;
var lineHeight = 22;
		...
		circle = theG.selectAll("circle")
						...
						.attr("cy", function (d,i){
							if(hoveredOneIndex < i){
								show("less");
								return ((hoveredOneIndex + 1 + i) * lineHeight / 2) + lineHeight;
						...
						//.attr("stroke", function (d,i){ return color(result[i].compat[hoveredOneIndex]); })
						//.attr("stroke-width", 1)
						.attr("fill", function (d,i){ return color(result[i].compat[hoveredOneIndex]); });
	}
	
	function undraw(theHoveredOne){
		circle.transition().attr("stroke", "grey").remove();
		theG.remove();
		$("#chart-area").remove();
